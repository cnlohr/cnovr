...
cnovr_canvas * shmcanvas;
int shmfile;
uint32_t * mmapeddata;
uint32_t * backupcanvaspointer;
int shmw;
int shmh;
...
{
	CNOVRCanvasSwapBuffers( shmcanvas );
//	printf( "%f %f\n", store->pOverlayLocation.Pos[0], store->pOverlayLocation.Pos[1] );
}
...
int OverlayFocusEvent( int event, cnovrfocus_capture * cap, cnovrfocus_properties * prop, int buttoninfo )
{
	if( event != CNOVRF_MOTION) 
		printf( "EVENT: %d\n", event );
	if( event == CNOVRF_UPFOCUS || event == CNOVRF_UPNOFOCUS )
	{
		printf( "SAVE SAVE %s %d\n", ptrname, store->initialized );
		CNOVRNamedPtrSave( ptrname );
	}

	return CNOVRCanvasFocusEvent( event, cap, prop, buttoninfo );
}



static void shmwindow_scene_setup( void * tag, void * opaquev )
{
	int splitnum;
	splits = CNOVRSplitStrings( identifier, ":", " ", 0, &splitnum ); //You can just free(...) the return. it's safe.
	if( splitnum != 3 )
	{
		ovrprintf( "Error: Identifier needs to be in format: <shmfile>:w:h\n" );
		return;
	}

	shmw = atoi(splits[1]);
	shmh = atoi(splits[2]);
	shmfile = open( splits[0], O_RDONLY );
	if( shmfile <= 0 )
	{
		ovrprintf( "Error: Could not open shmfile: %s\n", splits[0] );
		return;
	}
	mmapeddata = mmap (0, shmw*shmh*4, PROT_READ, MAP_SHARED, shmfile, 0);
	if( mmapeddata <= 0 )
	{
		ovrprintf( "Error: Could not map shmfile: %s\n", splits[0] );
		return;
	}

	ptrname = strdup( splits[0] );
	{ char * cc = ptrname; while( *cc != 0 ) { if( *(cc) == '/' ) *cc = '_'; cc++; } }
 
	store = CNOVRNamedPtrData( ptrname, 0, sizeof( *store ) + 1024 );
	printf( "Creating canvas: %s/%d/%d\n", splits[0], shmw, shmh );
	shmcanvas = CNOVRCanvasCreate( splits[0], shmw, shmh, CANVAS_PROP_IS_OPENVR_OVERLAY | CANVAS_PROP_FORCE_RGB );
	printf( "Canvas created\n" );

	//Swap out the framebuffer from under the canvas.
	backupcanvaspointer = shmcanvas->data;
	shmcanvas->data = mmapeddata+808; //?!?!?
	shmcanvas->model->pose = shmcanvas->pose = &store->pOverlayLocation;
	...
	overlaycapture.opaque = shmcanvas;
	overlaycapture.cb = OverlayFocusEvent;
	CNOVRModelSetInteractable( shmcanvas->model, &overlaycapture );
...
void start( const char * lidentifier )
{
	...
	quit = 0;
	CNOVRJobTack( cnovrQPrerender, shmwindow_scene_setup, 0, 0, 0 );
	printf( "=== shmwindow start %s(%p)\n", identifier, identifier );
}

void stop( const char * identifier )
{
	quit = 1;
//	if( mmapeddata ) munmap( mmapeddata, shmw*shmh*4 );
	if( shmfile ) close( shmfile );
	shmcanvas->data = backupcanvaspointer;
	printf( "=== End shmwindow stop\n" );
}
