// stb_include.h - v0.01(MOD) - parse and process #include directives - public domain
//
// Mods by Charles Lohr (CNLohr) still free, public domain... compare to stb_include.h v0.01
//
...
char *stb_include_string(char *str, char *inject, char *path_to_includes, char *filename, char error[256], void(*includecallback)( void * opaque, const char * filename ), void * opaque )
{
   ...
   for (i=0; i < num; ++i) {
      ...
      if (inc_list[i].filename == 0) {
         ...
         strcat(temp, "/");
         strcat(temp, inc_list[i].filename);
         inc = stb_include_file(temp, inject, path_to_includes, error, includecallback, opaque );
         if (inc == NULL) {
...
char *stb_include_strings(char **strs, int count, char *inject, char *path_to_includes, char *filename, char error[256], void(*includecallback)( void * opaque, const char * filename ), void * opaque )
{
   ...
   result = stb_include_string(text, inject, path_to_includes, filename, error, includecallback, opaque );
   free(text);
   return result;   
}
...
char *stb_include_file(char *filename, char *inject, char *path_to_includes, char error[256], void(*includecallback)( void * opaque1, const char * filename ), void * opaque )
{
   size_t len;
   char *result;
   includecallback( opaque, filename );
